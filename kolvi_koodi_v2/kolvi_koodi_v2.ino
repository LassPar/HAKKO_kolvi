...
float lue_lampotila()
{
  ...
  {
    if (raw_temp < thermistortable[i][1])
    {
      continue;
    }
    else if (raw_temp >= thermistortable[i][1] && raw_temp <= thermistortable[i+1][1])
    {
      return float(mapfloat(raw_temp, thermistortable[i][1], thermistortable[i+1][1], thermistortable[i][2], thermistortable[i+1][2]));
    }
    else
    {
      return 0;
...
void PID_cal()
{
  ...
  float temps[5] = {lue_lampotila(), temps[0], temps[1], temps[2], temps[3]};
